...
# =============================================
echo "[+] [Auth] Waiting for Vault..."
MAX_RETRIES=30
for i in $(seq 1 $MAX_RETRIES); do
  if curl -sf "$VAULT_ADDR/v1/sys/health" >/dev/null 2>&1; then
    echo "[+] [Auth] Vault is ready after $i attempts."
    break
  fi
  if [ "$i" -eq "$MAX_RETRIES" ]; then
    echo "[-] [Auth] ERROR: Vault did not become ready in time." >&2
    exit 1
  fi
  sleep 2
done
...
if [ -z "$VAULT_TOKEN" ] || [ "$VAULT_TOKEN" = "null" ]; then
  echo "[-] [Auth] CRITICAL ERROR: AppRole login failed." >&2
  exit 1
fi
...
echo "[+] [Auth] Fetching secrets from Vault..."
...
# =============================================
# Prisma migrations + seed (already running as nodejs: no su-exec)
# =============================================
